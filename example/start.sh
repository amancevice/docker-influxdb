...
docker run --rm --net influxdb --entrypoint curl \
    amancevice/influxdb -G http://ix0:8086/query --data-urlencode "q=CREATE DATABASE mydb" &> /dev/null

echo
echo "SHOW DATABASES:"
docker run --rm -it --net influxdb --entrypoint /usr/bin/influx \
    amancevice/influxdb -host ix0 -execute "SHOW DATABASES"
...
docker run --rm --net influxdb --entrypoint curl \
    amancevice/influxdb -i -XPOST 'http://ix0:8086/write?db=mydb' --data-binary 'cpu_load_short,host=server01,region=us-west value=0.64 1434055562000000000' &> /dev/null

echo
echo "SELECT * FROM cpu_load_short;"
docker run --rm -it --net influxdb --entrypoint /usr/bin/influx \
    amancevice/influxdb -host ix0 -database mydb -precision rfc3339 -execute "SELECT * FROM cpu_load_short;"
